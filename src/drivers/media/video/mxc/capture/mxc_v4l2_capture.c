 ...
 */
/*
 *  MODIFIED: Konstantyn Prokopenko, ParkAssist International, Sep 2013.
 *  - Added parallel driver stack channels for multiple camera sensors connected to 
  * the same IPU, different CSIs.
  * - Added DMAMAP memory type for buffer requests. This type remaps physical contig
  * memory areas tor sensor frames. The memory areas should be reserved at kernel boot
  * time by using command line arguments or by other means. 
 */
/*!
...
static unsigned long _dmamap_start_addr = PARALLAX_DMA_MEM_START;
static unsigned long _dmamap_buf_size = PARALLAX_DMA_BUF_SIZE;
...
/*!
 * Unmap fma map buffers
 *
 * @param cam      Structure cam_data *
 *
 * @return status  0 success.
 */
static int mxc_free_dmamap_buf(cam_data *cam)
{
	int i;

	pr_debug("MVC: In mxc_free_dmamap_buf\n");

	for (i = 0; i < FRAME_NUM; i++) {
		if (cam->frame[i].vaddress != NULL) {
			iounmap(cam->frame[i].vaddress);
			cam->frame[i].vaddress = NULL;
                        if(i < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
                            cam->snapshot.buffer_control[i] = 0;
                            if(cam->snapshot.buffer_count)
                                cam->snapshot.buffer_count--;
                        } 
		}
	}
	return 0;
...
/*!
 * Get frame buffer memory type
 *
 * @param cam      Structure cam_data *
 *
 * @return memory type.
 */
static enum v4l2_memory mxc_get_buf_type(cam_data *cam)
{
	pr_debug("MVC: In mxc_buf_type\n");
	/* Check the first frame only, the rest should be the same type */
	if (cam->frame[0].vaddress != NULL) {
		return cam->frame[0].buffer.memory;
	}
	return 0;
}
...
/*!
 * Allocate DMA MAP frame buffers
 *
 * @param cam      Structure cam_data*
 * @param boffset  Buffer offset to start from (in buffers).
 * @param count    int number of buffer need to remap
 *
 * @return status  -0 Successfully remapped a buffer, -ENOBUFS	failed.
 */
static int mxc_allocate_dmamap_buf(cam_data *cam, int boffset, int count)
{
	int i;
	unsigned long map_addr = _dmamap_start_addr + boffset*_dmamap_buf_size;

	printk(KERN_INFO "MXC V4L: mxc_allocate_dmabuf_buf - size=%lu, boffset: %d, DMAMAP start: %lX\n",
		_dmamap_buf_size, boffset, _dmamap_start_addr);

	for (i = 0; i < count; i++) {
		cam->frame[i].paddress = map_addr;
		cam->frame[i].buffer.length = PAGE_ALIGN(_dmamap_buf_size);		
		cam->frame[i].vaddress = ioremap(cam->frame[i].paddress, 
										 cam->frame[i].buffer.length);
		if (cam->frame[i].vaddress == NULL) {
			pr_err("ERROR: v4l2 capture: "
				"mxc_allocate_dmamap_buf failed to remap address: %lX, start address: %lX\n", map_addr, _dmamap_start_addr);
			mxc_free_dmamap_buf(cam);
			return -ENOBUFS;
		}
		cam->frame[i].buffer.index = i;
		cam->frame[i].buffer.flags = V4L2_BUF_FLAG_MAPPED;
		cam->frame[i].buffer.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
		cam->frame[i].buffer.memory = V4L2_MEMORY_DMAMAP;
		cam->frame[i].buffer.m.offset = cam->frame[i].paddress;
		cam->frame[i].index = i;
		map_addr += cam->frame[i].buffer.length;
                /* Snapshot parameters */
                if(i < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
                    cam->snapshot.buffer_control[i] = 
                         (PARALLAX_SNAPSHOT_BUF_TYPE_DMA |
                         PARALLAX_SNAPSHOT_BUF_MODE_ALLOC|PARALLAX_SNAPSHOT_BUF_MODE_MAPD);
                    cam->snapshot.buffer_count++;
                } 
	}
...
static int mxc_streamon(cam_data *cam)
{
	...
	if (cam->enc_update_eba) {
		...
		spin_unlock_irqrestore(&cam->queue_int_lock, lock_flags);
		printk("ERROR: encoder update eba is not set\n");
		return -EINVAL;
	}
...
/*!
 * Shut down the encoder job retaining frames
 *
 * @param cam      structure cam_data *
 *
 * @return status  0 Success
 */
static int mxc_streamoff_lazy(cam_data *cam)
{
	int err = 0;

	pr_debug("In MVC:mxc_streamoff_lazy\n");
...
static int mxc_v4l2_s_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		 ...
		 */
		if (strstr(mxc_capture_inputs[cam->current_input].name,
			   "CSI MEM") != 0) {
		...
		*height -= *height % 8;
		...
		case V4L2_PIX_FMT_SBGGR10:
                        printk(KERN_INFO "++++%s: PIXEL FORMAT: SBGGR10\n", __func__);
		...
		printk(KERN_INFO "mxc_v4l2_capture: size: %d, bytes_per_line: %d\n", size, bytesperline);
...
static int mxc_v4l2_g_ctrl(cam_data *cam, struct v4l2_control *c)
{
	...
	case V4L2_CID_BRIGHTNESS:
		if (cam->sensor) {
			...
			pr_err("ERROR: v4l2 capture: brightness: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_HUE:
		if (cam->sensor) {
			...
			pr_err("ERROR: v4l2 capture: hue: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_CONTRAST:
		if (cam->sensor) {
			...
			pr_err("ERROR: v4l2 capture: contrast: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_SATURATION:
		if (cam->sensor) {
			...
			pr_err("ERROR: v4l2 capture: saturation: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_RED_BALANCE:
		if (cam->sensor) {
			...
			pr_err("ERROR: v4l2 capture: red balance: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_BLUE_BALANCE:
		if (cam->sensor) {
			...
			pr_err("ERROR: v4l2 capture: blue balance: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_BLACK_LEVEL:
		if (cam->sensor) {
			...
			pr_err("ERROR: v4l2 capture: black level: slave not found!\n");
			status = -ENODEV;
		}
		break;
        case V4L2_CID_EXPOSURE:
                if (cam->sensor) {
                        ...
                        pr_err("ERROR: v4l2 capture: exposure: slave not found!\n");
			status = -ENODEV;
                }
                break;
        case V4L2_CID_AUTOEXPOSURE:
                if (cam->sensor) {
                        ...
                        pr_err("ERROR: v4l2 capture: auto-exposure: slave not found!\n");
			status = -ENODEV;
                }
                break;
        case V4L2_CID_AUTOGAIN:
                if (cam->sensor) {
                        ...
                        pr_err("ERROR: v4l2 capture: auto-gain: slave not found!\n");
			status = -ENODEV;
                }
                break;
        case V4L2_CID_GAIN:
                if (cam->sensor) {
                        ...
                        pr_err("ERROR: v4l2 capture: gain: slave not found!\n");
			status = -ENODEV;
                }
                break;
	default:
...
static int mxc_v4l2_s_ctrl(cam_data *cam, struct v4l2_control *c)
{
	...
	case V4L2_CID_BLUE_BALANCE:
		...
		break;
	case V4L2_CID_EXPOSURE:
                //printk(KERN_INFO "MXCV4L: V4L2_CID_EXPOSURE: Set Exposure: %d\n",c->value);
		if (cam->sensor) {
			cam->exposure = c->value;
			ret = vidioc_int_s_ctrl(cam->sensor, c);
		} else {
	...
	case V4L2_CID_GAIN:
        ...
        case V4L2_CID_AEC_POLYGON_LR:
                if (cam->sensor) {
                        ...
                        pr_err("ERROR: v4l2 capture: aec polygon: slave not found!\n");
                        ret = -ENODEV;
                }
                break;
			...
			for (i = 0; i < cam->sensor_index; i++) {
				if (i != c->value) {
					...
					vidioc_int_s_power(cam->all_sensors[i], 0);
                                        cam->snapshot.mxcv4l_control &= ~PARALLAX_SNAPSHOT_MXCV4L_POWER;
...
static int mxc_v4l2_s_param(cam_data *cam, struct v4l2_streamparm *parm)
{
	...
	if (cam->device_type != 1) {
                //printk(KERN_INFO "-=-=-=-=-=-=mxc_v4l2_s_param: setting crop current == crop_bounds\n");
		//cam->crop_current.width = cam->crop_bounds.width;
		//cam->crop_current.height = cam->crop_bounds.height;
		cam->crop_current.width = parm->fpos.width;
		cam->crop_current.height = parm->fpos.height;
		cam->crop_current.left = parm->fpos.xstart;
		cam->crop_current.top = parm->fpos.ystart;
	}
...
/*        printk(KERN_INFO "-=-=-=-=-=-=mxc_v4l2_s_param: setting crop: w/h: %d/%d\n", cam->crop_current.width, cam->crop_current.height); */
	ipu_csi_set_window_size(cam->ipu, cam->crop_current.width,
				cam->crop_current.height, cam->csi);
/*        printk(KERN_INFO "-=-=-=-=-=-=mxc_v4l2_s_param: setting pos: t/l: %d/%d\n", cam->crop_current.top, cam->crop_current.left); */
	ipu_csi_set_window_pos(cam->ipu, cam->crop_current.left,
			       cam->crop_current.top,
			       cam->csi);
/*        printk(KERN_INFO "++++%s: ipu_csi_init: pixel format: %X\n", __func__, cam_fmt.fmt.pix.pixelformat); */
...
/*!
 * V4L2 - mxc_v4l2_g_snapshot function
 *
 * Gets the snapshot structure to user.
 *
 * @param cam         structure cam_data *
 *
 * @param s           structure mxcv4l_kernel_snapshot *
 *
 * @return  status    0 success, EINVAL failed
 */
static int mxc_v4l2_g_snapshot(cam_data *cam, struct mxcv4loverlay_kernel_entry *s)
{
    memcpy(s, &cam->snapshot, sizeof(cam->snapshot));
    return 0;
}
...
static int mxc_v4l_dqueue(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	if (!wait_event_interruptible_timeout(cam->enc_queue,
		...
		pr_err(KERN_INFO "ERROR: v4l2 capture: mxc_v4l_dqueue timeout "
			"enc_counter %x\n",
		       cam->enc_counter);
                cam->snapshot.dqueue_err = -ETIME;
		return -ETIME;
	} else if (signal_pending(current)) {
/*                printk(KERN_INFO "Error: signal received: signals: %X, blocked: %X\n", current->signal,current->blocked); */
		pr_err(KERN_INFO "ERROR: v4l2 capture: mxc_v4l_dqueue() "
			"interrupt received\n");
                cam->snapshot.dqueue_err = -EAGAIN;
		return -EAGAIN;
		/* return -ERESTARTSYS; */
	}
        cam->snapshot.csi_interrupts++;
	...
	list_del(cam->done_q.next);
        if(frame->index < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
            cam->snapshot.buffer_control[frame->index] &= ~PARALLAX_SNAPSHOT_BUF_MODE_ERR;
        }
        cam->snapshot.dqueue_err = 0;
	if (frame->buffer.flags & V4L2_BUF_FLAG_DONE) {
		frame->buffer.flags &= ~V4L2_BUF_FLAG_DONE;
                if(frame->index < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
                    cam->snapshot.buffer_control[frame->index] &= 
                             ~(PARALLAX_SNAPSHOT_BUF_MODE_DONE | PARALLAX_SNAPSHOT_BUF_MODE_ERR);
                }
	} else if (frame->buffer.flags & V4L2_BUF_FLAG_QUEUED) {
		pr_err(KERN_INFO "ERROR: v4l2 capture: VIDIOC_DQBUF: "
			"Buffer not filled: %d, flags: %X.\n", frame->index, frame->buffer.flags);
		frame->buffer.flags &= ~V4L2_BUF_FLAG_QUEUED;
                if(frame->index < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
                    cam->snapshot.buffer_control[frame->index] &= ~PARALLAX_SNAPSHOT_BUF_MODE_QUED;
                    cam->snapshot.buffer_control[frame->index] |= PARALLAX_SNAPSHOT_BUF_MODE_ERR;
                    cam->snapshot.dqueue_err = PARALLAX_SNAPSHOT_DEQUE_ERROR_BNF;
                }
		retval = -EINVAL;
	} else if ((frame->buffer.flags & 0x7) == V4L2_BUF_FLAG_MAPPED) {
		pr_err(KERN_INFO "ERROR: v4l2 capture: VIDIOC_DQBUF: "
			"Buffer not queued.: %d, flags: %X\n", frame->index, frame->buffer.flags);
                if(frame->index < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
                    cam->snapshot.buffer_control[frame->index] |= PARALLAX_SNAPSHOT_BUF_MODE_ERR;
                    cam->snapshot.dqueue_err = PARALLAX_SNAPSHOT_DEQUE_ERROR_BNQ;
                }
		retval = -EINVAL;
	}
//        printk(KERN_INFO "++++++Dequeue image, size: %d\n", cam->v2f.fmt.pix.sizeimage);
...
static int mxc_v4l_open(struct file *file)
{
	...
	if (cam->open_count++ == 0) {
		...
		ipu_csi_set_window_pos(cam->ipu, cam->crop_current.left,
					cam->crop_current.top,
					cam->csi);
                printk(KERN_INFO "++++%s: open: pixel format: %X\n", __func__, cam_fmt.fmt.pix.pixelformat);
		ipu_csi_init_interface(cam->ipu, cam->crop_bounds.width,
					cam->crop_bounds.height,
...
static int mxc_v4l_close(struct file *file)
{
	...
	if (--cam->open_count == 0) {
		...
		if (strcmp(mxc_capture_inputs[cam->current_input].name,
			   "CSI MEM 0") == 0) {
#if defined(CONFIG_MXC_IPU_CSI_ENC) || defined(CONFIG_MXC_IPU_CSI_ENC_MODULE)
			err |= csi0_enc_deselect(cam);
#endif
		}
		else if (strcmp(mxc_capture_inputs[cam->current_input].name,
			   "CSI MEM 1") == 0) {
#if defined(CONFIG_MXC_IPU_CSI_ENC) || defined(CONFIG_MXC_IPU_CSI_ENC_MODULE)
			err |= csi1_enc_deselect(cam);
#endif
		} else if (strcmp(mxc_capture_inputs[cam->current_input].name,
				  "CSI IC MEM") == 0) {
#if defined(CONFIG_MXC_IPU_PRP_ENC) || defined(CONFIG_MXC_IPU_PRP_ENC_MODULE)
		...
		if(mxc_get_buf_type(cam) & V4L2_MEMORY_DMAMAP)
			mxc_free_dmamap_buf(cam);
		else
			mxc_free_frame_buf(cam);
			    ...
			    loff_t *ppos)
{
	...
	if(cam->csi == 0)
		err = prp_csi0_still_select(cam);
	else
		err = prp_csi1_still_select(cam);
	if (err != 0) {
		err = -EIO;
		goto exit0;
	}
...
//	printk(KERN_INFO "mxc_v4l2_capture: IOCTL: %X\n", ioctlnr);
	...
	case VIDIOC_REQBUFS: {
		...
		if (req->memory & V4L2_MEMORY_DMAMAP) {
			retval = mxc_allocate_dmamap_buf(cam, req->boffset, req->count);
		}
		break;
	}
	...
	case VIDIOC_QUERYBUF: {
		...
		if (buf->memory & (V4L2_MEMORY_MMAP | V4L2_MEMORY_DMAMAP)) {
	...
	case VIDIOC_QBUF: {
		...
		if ((cam->frame[index].buffer.flags & 0x7) ==
			...
			cam->frame[index].buffer.flags |=
			    V4L2_BUF_FLAG_QUEUED;
                        if (index < PARALLAX_VPU_BUF_NUM) {
                            cam->snapshot.buffer_control[index] |= PARALLAX_SNAPSHOT_BUF_MODE_QUED;
			...
			pr_err(KERN_INFO "ERROR: v4l2 capture: VIDIOC_QBUF: "
			       "buffer already queued: %d, flags: %X\n", index, cam->frame[index].buffer.flags);
                        if (index < PARALLAX_VPU_BUF_NUM)
                            cam->snapshot.buffer_control[index] |= PARALLAX_SNAPSHOT_BUF_MODE_ERR;
                        cam->snapshot.dqueue_err = PARALLAX_SNAPSHOT_DEQUE_ERROR_BAQ;
			retval = -EINVAL;
		} else if (cam->frame[index].buffer.
			...
			cam->frame[index].buffer.flags |=
			    V4L2_BUF_FLAG_QUEUED;
                        if (index < PARALLAX_VPU_BUF_NUM) {
                            cam->snapshot.buffer_control[index] &= ~PARALLAX_SNAPSHOT_BUF_MODE_DONE;
                            cam->snapshot.buffer_control[index] |= 
                                  (PARALLAX_SNAPSHOT_BUF_MODE_QUED | PARALLAX_SNAPSHOT_BUF_MODE_ERR);
                            cam->snapshot.dqueue_err = PARALLAX_SNAPSHOT_DEQUE_ERROR_ODB;
                        }
			retval = -EINVAL;
		}
        ...
        case VIDIOC_SNAPSHOT: {
//            printk(KERN_INFO "====Snapshot requested. Buffers: %d, csi: %d, ipu: %d, ctrl: %X\n", cam->snapshot.buffer_count,
 //                  cam->snapshot.csi_id, cam->snapshot.ipu_id, cam->snapshot.mxcv4l_control);
            retval = mxc_v4l2_g_snapshot(cam, arg);
            break;
        }
	...
	case VIDIOC_DQBUF: {
		...
		if ((cam->enc_counter == 0) &&
		    ...
		    retval = mxc_v4l_dqueue(cam, buf);
                    if(retval != 0) {
                        printk(KERN_INFO "ERROR from DQBUF, retval: %d\n", retval);
                        if(retval != -EAGAIN)
                            break;
                        printk(KERN_INFO "DQBUF: Retrying....\n");
		    }
                    else
                        break;
                    dque_count++;
	...
	case VIDIOC_STREAMOFF_LAZY: {
		pr_debug("   case VIDIOC_STREAMOFF_LAZY\n");
		retval = mxc_streamoff_lazy(cam);
		break;
	}
	...
	case VIDIOC_S_CROP: {
		...
		pr_debug("   Cropping Input to ipu size %d x %d\n",
				...
				cam->crop_current.height);
        	//printk(KERN_INFO "-=-=-=-=-=-=VIDEOC_S_CROP: setting size: w/h: %d/%d\n", cam->crop_current.width, cam->crop_current.height);
        	//printk(KERN_INFO "-=-=-=-=-=-=VIDEOC_S_CROP: setting pos: t/l: %d/%d\n", cam->crop_current.top, cam->crop_current.left);
		ipu_csi_set_window_size(cam->ipu, cam->crop_current.width,
					cam->crop_current.height,
					cam->csi);
		ipu_csi_set_window_pos(cam->ipu, cam->crop_current.left,
	...
	case VIDIOC_G_PARM: {
		...
		if (cam->sensor)
			...
			pr_err("ERROR: v4l2 capture: G parm: slave not found!\n");
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_S_PARM:  {
		struct v4l2_streamparm *parm = arg;
      //          struct v4l2_framepos *fpos = (struct v4l2_framepos *)&parm->parm.raw_data[0];
		pr_debug("   case VIDIOC_S_PARM\n");
        //        printk(KERN_INFO "====MXCV4L2: fpos: X: %d, Y: %d, W: %d, H: %d\n", fpos->xstart, fpos->ystart,
        //               fpos->width, fpos->height);
		if (cam->sensor)
			retval = mxc_v4l2_s_param(cam, parm);
		else {
			pr_err("ERROR: v4l2 capture: S PARM: slave not found!\n");
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_G_STD: {
		...
		if (cam->sensor)
			...
			pr_err("ERROR: v4l2 capture: G STD: slave not found!\n");
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_S_INPUT: {
		...
		if (strcmp(mxc_capture_inputs[*index].name, "CSI MEM 0") == 0) {
#if defined(CONFIG_MXC_IPU_CSI_ENC) || defined(CONFIG_MXC_IPU_CSI_ENC_MODULE)
			retval = csi0_enc_select(cam);
			if (retval)
				break;
#endif
		}
		if (strcmp(mxc_capture_inputs[*index].name, "CSI MEM 1") == 0) {
#if defined(CONFIG_MXC_IPU_CSI_ENC) || defined(CONFIG_MXC_IPU_CSI_ENC_MODULE)
			retval = csi1_enc_select(cam);
			if (retval)
				break;
#endif
	...
	case VIDIOC_ENUM_FMT: {
		...
		if (cam->sensor)
			...
			pr_err("ERROR: v4l2 capture: ENUM FMT: slave not found!\n");
			retval = -ENODEV;
		}
		break;
	}
	case VIDIOC_ENUM_FRAMESIZES: {
		...
		if (cam->sensor)
			...
			pr_err("ERROR: v4l2 capture: ENUM FRAMESIZES: slave not found!\n");
			retval = -ENODEV;
		}
		break;
	}
	case VIDIOC_DBG_G_CHIP_IDENT: {
		...
		if (cam->sensor)
			...
			pr_err("ERROR: v4l2 capture: G CHIP IDENT: slave not found!\n");
			retval = -ENODEV;
		}
		break;
	}
	...
	default:
		printk(KERN_INFO "mxc_v4l2_capture: IOCTL is not supported\n");
	...
	if (!list_empty(&cam->working_q)) {
		...
		if (done_frame->buffer.flags & V4L2_BUF_FLAG_QUEUED) {
			...
			done_frame->buffer.flags &= ~V4L2_BUF_FLAG_QUEUED;
                        if(done_frame->index < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
                            cam->snapshot.buffer_control[done_frame->index] &= ~PARALLAX_SNAPSHOT_BUF_MODE_QUED;
                            cam->snapshot.buffer_control[done_frame->index] |= PARALLAX_SNAPSHOT_BUF_MODE_DONE;
                            cam->snapshot.buffer_control[done_frame->index] &= ~PARALLAX_SNAPSHOT_BUF_MODE_ERR;
                        } 
			...
			pr_err(KERN_INFO "ERROR: v4l2 capture: camera_callback: "
				"buffer not queued: %d, flags: %X\n", done_frame->index, done_frame->buffer.flags);
                        if(done_frame->index < PARALLAX_SNAPSHOT_MAX_BUFFERS) {
                            cam->snapshot.buffer_control[done_frame->index] |= PARALLAX_SNAPSHOT_BUF_MODE_ERR;
                            cam->snapshot.dqueue_err = PARALLAX_SNAPSHOT_DEQUE_ERROR_CBNQ;
                        }
               }
	}
...
//	cam->crop_bounds.width = 640;
	cam->crop_bounds.width = 2592;
	cam->crop_bounds.top = 0;
	cam->crop_bounds.height = 1944;
//	cam->crop_bounds.height = 480;
	cam->crop_current = cam->crop_defrect = cam->crop_bounds;
       	//printk(KERN_INFO "-=-=-=-=-=-=init: setting size: w/h: %d/%d\n", cam->crop_current.width, cam->crop_current.height);
       	//printk(KERN_INFO "-=-=-=-=-=-=init: setting pos: t/l: %d/%d\n", cam->crop_current.top, cam->crop_current.left);
	ipu_csi_set_window_size(cam->ipu, cam->crop_current.width,
				cam->crop_current.height, cam->csi);
	ipu_csi_set_window_pos(cam->ipu, cam->crop_current.left,
...
//	cam->v2f.fmt.pix.pixelformat = V4L2_PIX_FMT_UYVY;
	cam->v2f.fmt.pix.pixelformat = V4L2_PIX_FMT_SBGGR10;
	cam->win.w.width = 160;
	cam->win.w.height = 160;
	cam->win.w.left = 0;
	cam->win.w.top = 0;

        printk(KERN_INFO "mxc_v4l2_capture: Initializing camera structure: csi: %d, ipu: %d\n", pdata->csi, pdata->ipu);
	cam->ipu_id = pdata->ipu;
	cam->csi = pdata->csi;
        cam->snapshot.ipu_id = cam->ipu_id;
        cam->snapshot.csi_id = cam->csi;
...
//	printk("MXC_V4l: init camera struct, name: %s\n", cam->self->name);
...
static int mxc_v4l2_probe(struct platform_device *pdev)
{
	...
	//printk("MXC_V4L: Probing video device\n");
...
static int mxc_v4l2_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	cam->low_power = true;
        cam->snapshot.mxcv4l_control |= PARALLAX_SNAPSHOT_MXCV4L_SUSPEND;
...
static int mxc_v4l2_master_attach(struct v4l2_int_device *slave)
{
	...
	if (sdata->csi != cam->csi) {
		printk("%s: csi doesn't match: sdata_csi: %d, cam_csi: %d\n", __func__, sdata->csi, cam->csi);
	...
	//printk(KERN_INFO "MXC_V4L: Master Attach done\n");
	return 0;
}
...
static void mxc_v4l2_master_detach(struct v4l2_int_device *slave)
{
	...
	cam->sensor_index--;
	//printk(KERN_INFO "MXC_V4L: Master Detach done: %s\n", slave->name);
