 ...
 * @file ipu_csi1_enc.c
 *
 * @brief CSI1 Use case for video capture
 *
 * @ingroup IPU
 */
...
static int csi_enc_enabling_tasks(void *private)
{
	...
	err = ipu_request_irq(cam->ipu, IPU_IRQ_CSI1_OUT_EOF,
			      csi_enc_callback, 0, "Mxc Camera 1", cam);
	if (err != 0) {
		return err;
	}
 ...
 * @return  int
 */
int csi1_enc_select(void *private)
{
	cam_data *cam = (cam_data *) private;
	int err = 0;
 ...
 * @return  int
 */
int csi1_enc_deselect(void *private)
{
	cam_data *cam = (cam_data *) private;
	int err = 0;
 ...
 *
 */
void __exit csi1_enc_exit(void)
{
}

module_init(csi1_enc_init);
module_exit(csi1_enc_exit);

EXPORT_SYMBOL(csi1_enc_select);
EXPORT_SYMBOL(csi1_enc_deselect);

MODULE_AUTHOR("Freescale Semiconductor, Inc.");
MODULE_DESCRIPTION("CSI1 ENC Driver");
MODULE_LICENSE("GPL");
