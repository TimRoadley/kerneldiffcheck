...
{
	cam_data *cam = devid;
	ipu_select_buffer(cam->ipu, CSI_MEM0, IPU_OUTPUT_BUFFER,
			  callback_flag%2 ? 1 : 0);
	if (callback_flag == 0)
		ipu_enable_channel(cam->ipu, CSI_MEM0);
...
static irqreturn_t prp_still_callback(int irq, void *dev_id)
{
	...
	if (callback_eof_flag < 5) {
		...
		ipu_select_buffer(cam->ipu, CSI_MEM0, IPU_OUTPUT_BUFFER, buffer_num);
#endif
	} else {
		cam->still_counter++;
...
static int prp_still_stop(void *private)
{
	...
	ipu_disable_channel(cam->ipu, CSI_MEM0, true);
 ...
 * @return  status
 */
int prp_csi0_still_select(void *private)
{
	cam_data *cam = (cam_data *) private;
	printk("IPU STILL: SELECT CSI0 channel\n");
	if (cam) {
 ...
 * @return  status
 */
int prp_csi0_still_deselect(void *private)
{
	cam_data *cam = (cam_data *) private;
	int err = 0;
...
EXPORT_SYMBOL(prp_csi0_still_select);
EXPORT_SYMBOL(prp_csi0_still_deselect);
