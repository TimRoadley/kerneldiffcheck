...
{
	cam_data *cam = devid;
	ipu_select_buffer(cam->ipu, CSI_MEM1, IPU_OUTPUT_BUFFER,
			  callback_flag%2 ? 1 : 0);
	if (callback_flag == 0)
		ipu_enable_channel(cam->ipu, CSI_MEM1);
...
static irqreturn_t prp_still_callback(int irq, void *dev_id)
{
	...
	if (callback_eof_flag < 5) {
		...
		ipu_select_buffer(cam->ipu, CSI_MEM1, IPU_OUTPUT_BUFFER, buffer_num);
#endif
	} else {
		cam->still_counter++;
...
static int prp_still_start(void *private)
{
	...
	//printk(KERN_INFO "IPU_STILL: start still, pixelformat: %d\n", cam->v2f.fmt.pix.pixelformat);
	...
	//	printk(KERN_INFO "IPU_STILL: format YUYV\n");
	}
...
#ifdef CONFIG_MXC_IPU_V1
//	printk(KERN_INFO "IPU_STILL: MXC_IPU_V1\n");
...
//	printk(KERN_INFO "IPU_STILL: ipu_request_irq\n");
...
#else
//	printk(KERN_INFO "IPU_STILL: MXC_IPU_OTHER\n");
	callback_eof_flag = 0;
	buffer_num = 0;

	ipu_clear_irq(cam->ipu, IPU_IRQ_CSI1_OUT_EOF);
//	printk(KERN_INFO "IPU_STILL: request irq\n");
	err = ipu_request_irq(cam->ipu, IPU_IRQ_CSI1_OUT_EOF, prp_still_callback,
			      0, "Mxc Camera", cam);
	if (err != 0) {
		printk(KERN_ERR "Error registering irq.\n");
		return err;
	...
	ipu_enable_channel(cam->ipu, CSI_MEM1);
//	printk(KERN_INFO "IPU_STILL: ipu_enable_csi\n");
...
#else
	ipu_free_irq(cam->ipu, IPU_IRQ_CSI1_OUT_EOF, cam);
#endif

	ipu_disable_csi(cam->ipu, cam->csi);
	ipu_disable_channel(cam->ipu, CSI_MEM1, true);
 ...
 * @return  status
 */
int prp_csi1_still_select(void *private)
{
	cam_data *cam = (cam_data *) private;
	printk("IPU STILL: SELECT CSI1 channel\n");
	if (cam) {
 ...
 * @return  status
 */
int prp_csi1_still_deselect(void *private)
{
	cam_data *cam = (cam_data *) private;
	int err = 0;
...
EXPORT_SYMBOL(prp_csi1_still_select);
EXPORT_SYMBOL(prp_csi1_still_deselect);
