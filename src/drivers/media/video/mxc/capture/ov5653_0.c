...
//	{0x3a0f, 0x38, 0, 0}, {0x3a10, 0x30, 0, 0}, 
        {0x3a1b, 0x3a, 0, 0},
...
static int ov5653_set_idle_mode(void) {
  ...
  printk(KERN_INFO "<< OV5653 0: SET IDLE MODE>>\n");
			    ...
			    enum ov5653_mode mode, enum ov5653_mode orig_mode)
{
  ...
  printk(KERN_INFO "<< OV5653 0: INIT MODE>>\n");
...
if (mode > ov5653_mode_MAX || mode < ov5653_mode_MIN) {
  ...
  printk(KERN_INFO "<< OV5653 0: INIT MODE FOR Width: %d, Height: %d>>\n", ov5653_data.pix.width, ov5653_data.pix.height);
...
//			msleep(Delay_ms);
 ...
 // dump_registers();
  // Wait a little
  printk(KERN_INFO "<< OV5653 0: MODE INIT DONE >>\n");
...
//    explines = (((exposure << 11) + (exposure << 8) ) >> 14); // Values are approximate
    ...
    printk(KERN_INFO "OV5653_0: Set Exposure: %d gain: %d\n",explines, gain);
...
static unsigned int ov5653_get_exposure(void)
{
	...
	exposure = ((exposure << 8) | regval);
//        printk(KERN_INFO "OV5653_0: [Got exposure: %d]\n", exposure);
	return exposure;
}
...
static int ov5653_set_gain(unsigned int gain)
{
  ...
  printk(KERN_INFO "<< OV5653 0: IOCTL SET GAIN: %d>>\n", gain);
...
void ov5653_set_aec_polygon_lr(__u32 pt)
{
   ...
   // Completed combined polygon, set it by reinitilizing sensor mode table
    printk(KERN_INFO "OV5653 0: Setting mode: %d\n", ov5653_data.streamcap.capturemode);
...
static int ioctl_dev_init(struct v4l2_int_device *s)
{
	...
	enum ov5653_frame_rate frame_rate;
        ...
        printk(KERN_INFO "<< OV5653 0: -=-=-=-=- DEVICE INIT-=-=-=-=-=>>\n");
	ov5653_data.on = true;
			...
			const struct i2c_device_id *id)
{
	...
	printk("OV5653_0: Probing sensor on CSI: %d, clock: %d\n", ov5653_data.csi, ov5653_data.mclk_source);
...
//		plat_data->pwdn(1);
	...
	pr_info("Camera ov5653_0 is found\n");
//	if (plat_data->pwdn)
//		plat_data->pwdn(1);
	return retval;
 ...
 * @return  Error code indicating success or failure
 */
static __init int ov5653_0_init(void)
{
	u8 err;
 ...
 * @return  Error code indicating success or failure
 */
static void __exit ov5653_0_clean(void)
{
	i2c_del_driver(&ov5653_i2c_driver);
}

module_init(ov5653_0_init);
module_exit(ov5653_0_clean);

MODULE_AUTHOR("Freescale Semiconductor, Inc.");
MODULE_DESCRIPTION("OV5653 0 Camera Driver");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0");
MODULE_ALIAS("CSI");
