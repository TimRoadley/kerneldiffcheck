...
#define IMX2_WDT_DEFAULT_TIME	12		/* in seconds */
...
static void imx2_wdt_timer_ping(unsigned long arg)
{
	...
	imx2_wdt_ping();
	mod_timer(&imx2_wdt.timer, jiffies + imx2_wdt.timeout * HZ / 3);
}
...
#if 0
        // Remove timer enable/disable. Watchdog is triggered in user space
...
static int __init imx2_wdt_probe(struct platform_device *pdev)
{
	...
	res_size = resource_size(res);
        printk(KERN_INFO "WATCHDOG: Memory Resource addr: %X, size: %d, name: %s\n", 
               res->start, res_size, res->name);
