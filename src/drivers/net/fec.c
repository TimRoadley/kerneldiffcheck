...
static struct ksz8873_smi phy_smi;
...
fec_enet_rx(struct net_device *ndev)
{
	...
	while (!((status = bdp->cbd_sc) & BD_ENET_RX_EMPTY)) {
		...
		if (status & (BD_ENET_RX_LG | BD_ENET_RX_SH | BD_ENET_RX_NO |
			...
			if (status & BD_ENET_RX_CR) {	/* CRC Error */ 
//				printk(KERN_INFO "FEC RX read: CRC errors, stat: %X, r_ctrl: %X, w_rctrl: %X\n", 
//						status, readl(fep->hwp + FEC_R_CNTRL), readl(fep->hwp + FEC_X_CNTRL));
				ndev->stats.rx_crc_errors++;
			}
		...
		if (status & BD_ENET_RX_CL) {
			printk(KERN_INFO "FEC: ERROR late collisions, stat: %X\n", status);
			ndev->stats.rx_errors++;
			ndev->stats.rx_frame_errors++;
			goto rx_processing_done;
		}
...
//	printk(KERN_INFO "FEC INTERRUPT, irq: %d\n", irq);
	do {
		int_events = readl(fep->hwp + FEC_IEVENT);
//		printk(KERN_INFO "FEC INTERRUPT: events: %d\n", int_events);
		writel(int_events, fep->hwp + FEC_IEVENT);

		if (int_events & FEC_ENET_RXF) {
//			printk(KERN_INFO "FEC INTERRUPT: event RXF\n");
			ret = IRQ_HANDLED;
			spin_lock_irqsave(&fep->hw_lock, flags);
...
//			printk(KERN_INFO "FEC INTERRUPT: event TXF\n");
			ret = IRQ_HANDLED;
			fec_enet_tx(ndev);
		}

		if (int_events & FEC_ENET_TS_TIMER) {
//			printk(KERN_INFO "FEC INTERRUPT: event TIMER\n");
			ret = IRQ_HANDLED;
...
//			printk(KERN_INFO "FEC INTERRUPT: event MII\n");
			ret = IRQ_HANDLED;
...
//#define MICREL_USE_MDIO 1

static int fec_enet_mdio_read(struct mii_bus *bus, int mii_id, int regnum)
{
#ifndef MICREL_USE_MDIO
	u32 data = 0;
	if(ksz8873_smi_read_reg(&phy_smi, mii_id, regnum, &data) !=0)
		return -1;
	return data;
#else
	...
	/* return value */
	retval = FEC_MMFR_DATA(readl(fep->hwp + FEC_MII_DATA));
	printk(KERN_INFO "FEC: Read MDIO: value: %X\n", retval);
	msleep(10000);
	return retval;
#endif
}
...
{
#ifndef MICREL_USE_MDIO
	return ksz8873_smi_write_reg(&phy_smi, mii_id, regnum, value);
#else
	struct fec_enet_private *fep = bus->priv;
	unsigned long time_left;
...
/* KP: Added SMI read */
static int fec_enet_mdio_read_smi(struct mii_bus *bus, int regnum)
{
#ifndef MICREL_USE_MDIO
        u32 data = 0;
        if(ksz8873_smi_read_reg(&phy_smi, 3, regnum, &data) !=0)
                return -1;
        return data;
#else
	struct fec_enet_private *fep = bus->priv;
        unsigned int man_reg = 0;
	unsigned long time_left;
...
#if 0
	man_reg = 0xFFFFFFFF;
        writel(man_reg, fep->hwp + FEC_MII_DATA);
        /* wait for end of transfer */
        time_left = wait_for_completion_timeout(&fep->mdio_done,
                        msecs_to_jiffies(FEC_MII_TIMEOUT));
        if (time_left == 0) {
        ...
        init_completion(&fep->mdio_done);
#endif
        /* SMI frames. We are not using MACB macros, just shift bits here */
        man_reg = (1L << 30);      /* SOF bit 01 */
//        man_reg |= (3 << 28);   /* Change opcode for non-MII operation -read Freescale bits 11 */
        man_reg |= (1 << 27);   /* Read code embedded in PHY address - first bit */
        man_reg |= ((regnum & 0xFF) << 18); /* Register number goes to the 3 bits of PHY addr and 5 bits of register addr */
        man_reg |= (2 << 16);   /* Turn around bits 10 has to be set according to Micrel spec */
        /* 
        * Now, here is the trick. Apparentlyi Micrel  chip thinks that this frame is invalid because op code is not 01 or 10
        * and do not reverse direction of MDIO pin after read frame is done. Writing FFFF at the end of the frame in DATA location
        * seams to fix the invalid frame. Logical 0 is about 0.5 volts above in read data coming back from Micrel but this will work.
        */
        man_reg |= 0x000000ff;
        ...
        /* return value */
        man_reg = FEC_MMFR_DATA(readl(fep->hwp + FEC_MII_DATA));
	printk(KERN_INFO "++++++FEC: Reading register: %X, data: %X\n", regnum, man_reg);
	return man_reg;
#endif
}

/* KP: SMI write register */
static int fec_enet_mdio_write_smi(struct mii_bus *bus, int regnum, u16 value)
{
#ifndef MICREL_USE_MDIO
	return ksz8873_smi_write_reg(&phy_smi, 3, regnum, value);
#else
	struct fec_enet_private *fep = bus->priv;
        unsigned int man_reg = 0;
	unsigned long time_left;
...
#endif
        /* SMI frames */
        man_reg = (1L << 30);              /* SOF bits 01 */
                                        /* Non standard frame op code 00 */
                                        /* 0 as first bit of the PHY addr for write op */
        man_reg |= (regnum << 18);      /* 8 bits of register number starts from 3 bits of PHY address */
        man_reg |= (2 << 16);           /* Turn around bit 10 has to be set according to Micrel spec */
        man_reg |= (value & 0xFF);      /* Data to the lower byte of DATA field. Next byte is 0 for SMI */

	printk(KERN_INFO "++++++FEC: initiate  write op: register: %X, value: %X, data: %X\n", regnum, value, man_reg);
        /* start a write op */
	writel(man_reg, fep->hwp + FEC_MII_DATA);
...
static int fec_enet_mii_probe(struct net_device *ndev)
{
	...
	snprintf(phy_name, MII_BUS_ID_SIZE, PHY_ID_FMT, mdio_bus_id, phy_id);
        if (fep->mii_bus->phy_map[phy_id]) {
                phy_dev = fep->mii_bus->phy_map[phy_id];
		printk(KERN_INFO "[PARALLAX]: Found host PHY\n");
        }
        else {
		...
		printk(KERN_INFO "[PARALLAX]: Connecting RMII PHY\n");
                phy_dev = ksz8873_connect(ndev, phy_name,  &fec_enet_adjust_link, 
					0, PHY_INTERFACE_MODE_RMII);
        } else {
		printk(KERN_INFO "[PARALLAX]: Connecting MII PHY\n");
                phy_dev = ksz8873_connect(ndev, phy_name,  &fec_enet_adjust_link, 
...
static int fec_enet_mii_init(struct platform_device *pdev, int phy_irq)
{
	...
	printk(KERN_INFO "FEC: MII Init, IRQ: %d\n", phy_irq);
	/* INIT SMI */
#ifndef MICREL_USE_MDIO
	ksz8873_smi_init(&phy_smi);
	printk(KERN_INFO "FEC: KSZ8873 SMI initialized\n");
#endif
	/*
	 ...
	 * second interface is added primarily for Switchmode.
	 *
	...
	fep->mii_bus->read_smi = fec_enet_mdio_read_smi;
	fep->mii_bus->write_smi = fec_enet_mdio_write_smi;
...
fec_enet_open(struct net_device *ndev)
{
	...
	printk(KERN_INFO "[PARALLAX]: Opening FEC enet\n");
	if (fep->use_napi)
	...
	printk(KERN_INFO "[PARALLAX]: FEC MII probe\n");
...
fec_restart(struct net_device *dev, int duplex)
{
	...
	/* Enable RMII mode */
	if (duplex) {
		/* RMII enable / FD enable */
		writel(OPT_FRAME_SIZE | 0x104, fep->hwp + FEC_R_CNTRL);
		writel(0x04, fep->hwp + FEC_X_CNTRL);
	} else {
		/* RMII enable / No Rcv on Xmit */
		writel(OPT_FRAME_SIZE | 0x106, fep->hwp + FEC_R_CNTRL);
...
fec_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "[PARALLAX]: FEC device probe\n");
	...
	irq = ndev->irq = gpio_to_irq(pdata->gpio_irq);
	printk(KERN_INFO "FEC: Got GPIO: %d, IRQ: %d\n", pdata->gpio_irq, ndev->irq);
	ret = request_irq(ndev->irq, fec_enet_interrupt,
			IRQF_TRIGGER_RISING,
			 pdev->name, ndev);
	if (ret) {
		printk(KERN_INFO "FEC: Unable to request GPIO IRQ: %d\n", ndev->irq);
		goto failed_irq;
	}
#else
	...
	printk(KERN_INFO "FEC: got IRQ: %d\n", ndev->irq);
        ret = request_irq(ndev->irq, fec_enet_interrupt, IRQF_DISABLED,
                          pdev->name, ndev);
        if (ret) {
                printk(KERN_ERR
                       "%s: Unable to request IRQ %d (error %d)\n",
                       pdev->name, ndev->irq, ret);
                goto failed_irq;
        }
#endif
	...
	printk(KERN_INFO "[PARALLAX]: FEC probe done\n");
	return 0;
