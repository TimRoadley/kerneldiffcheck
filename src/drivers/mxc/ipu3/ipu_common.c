...
static int __devinit ipu_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "MXC IPU3: IPU Probe: Probing IPU ID: %d\n", pdev->id);
	...
	ipu->online = true;
	printk(KERN_INFO "MXC IPU3: IPU %d is registered\n", pdev->id);
...
//	printk(KERN_INFO "MXC IPU: IPU INIT: ipu_config reg: %X\n", ipu_conf);
//	ipu_conf |= IPU_CONF_CSI0_DATA_SOURCE; /* KP: hardcode to CSI0 for IPU */
...
//	printk(KERN_INFO "MXC IPU3: Enable CSI: %d\n", csi);
	if (csi > 1) {
	...
	_ipu_get(ipu);
//	printk(KERN_INFO "MXC IPU3: Got IPU\n");
	mutex_lock(&ipu->mutex_lock);
	ipu->csi_use_count[csi]++;
//	printk(KERN_INFO "MXC IPU3: Got usecount: %d\n", ipu->csi_use_count[csi]);

	if (ipu->csi_use_count[csi] == 1) {
//		printk(KERN_INFO "MXC IPU3: we are owners\n");
		reg = ipu_cm_read(ipu, IPU_CONF);
//		printk(KERN_INFO "MXC IPU3: config register: %X\n", reg);
		if (csi == 0) {
//			ipu_cm_write(ipu, reg | IPU_CONF_CSI0_EN | IPU_CONF_CSI0_DATA_SOURCE, IPU_CONF);
			reg &= ~IPU_CONF_CSI0_DATA_SOURCE;
			ipu_cm_write(ipu, reg | IPU_CONF_CSI0_EN, IPU_CONF);
		}
		else {
			reg &= ~IPU_CONF_CSI1_DATA_SOURCE;
			ipu_cm_write(ipu, reg | IPU_CONF_CSI1_EN, IPU_CONF);
		}
		reg = ipu_cm_read(ipu, IPU_CONF);
//		printk(KERN_INFO "MXC IPU3: config registerafter enable: %X\n", reg);
	}
	mutex_unlock(&ipu->mutex_lock);
//	printk(KERN_INFO "MXC IPU3: release IPU\n");
	_ipu_put(ipu);
	return 0;
}
...
//	printk(KERN_INFO "IMX IPU3: request IRQ: %d, name: %s, handler: %p\n", irq, devname, ipu->irq_list[irq].handler);
