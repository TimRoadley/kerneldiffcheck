...
//	printk(KERN_INFO "MXC IPU: get_irq: chan: %d\n", chan);
	switch (chan) {
	case MEM_ROT_VF_MEM:
...
//	printk(KERN_INFO "MXC IPU: do_task\n");
	if (!ipu) {
	...
	t->irq = irq;
//	printk(KERN_INFO "MXC IPU: do_task, irq: %d\n", irq);
...
int register_ipu_device(struct ipu_soc *ipu, int id)
{
	...
	if (!major) {
		major = register_chrdev(MXC_IPU_MAJOR, "mxc_ipu", &mxc_ipu_fops);
		if (major < 0) {
