    ...
    )
{
    ...
    printk(KERN_INFO "GPU: Open Driver\n");
...
#endif
{
    ...
    gcmkHEADER();
    printk(KERN_INFO "GPU: Init Driver\n");
...
# endif
    ...
    printk(KERN_INFO "GPU: Creating device driver inode galcore: %d,%d\n", major,0);
...
static int __devinit gpu_probe(struct platform_device *pdev)
{
    ...
    if (contiguousSize == 0)
       gckOS_Print("Warning: No contiguous memory is reserverd for gpu.!\n ");
        ...
        printk(KERN_INFO "GPU: Using reserved memory for GPU operations: %lX, size: %lu\n", contiguousBase, contiguousSize);
    }
