...
static struct clk video_27M_clk_root;
...
#if 0
static struct clk video_27M_clk_root = {
        __INIT_CLK_DEBUG(video_27M_clk_root)
        .parent = &periph_clk,
        .set_parent = _clk_v27M_set_parent,
        .set_rate = _clk_v27M_set_rate,
        .get_rate = _clk_v27M_get_rate,
        .round_rate = _clk_v27M_round_rate,
};
#endif
...
static int _clk_clko_set_parent(struct clk *clk, struct clk *parent)
{
	...
	if (parent == &pll3_usb_otg_main_clk)
        ...
        printk(KERN_INFO "IMX6 CLOCK: Setting CLKO selection: %d\n", sel);
	reg = __raw_readl(MXC_CCM_CCOSR);
	reg &= ~(MXC_CCM_CCOSR_CKOL_MIRROR_CKO2_MASK |
...
static int _clk_clko_set_rate(struct clk *clk, unsigned long rate)
{
	...
	if (((parent_rate / div) != rate) || (div > 8))
        ...
        printk(KERN_INFO "IMX6 CLOCK: Setting CLKO rate. Div: %d, rate: %d, parent rate: %d\n", div-1, rate, parent_rate);
	reg = __raw_readl(MXC_CCM_CCOSR);
